...
import { getEnrollments } from 'apiSdk/enrollments';
import { EnrollmentInterface } from 'interfaces/enrollment';

import { TutorListPage as ShowCaseList } from 'pages/tutors';
...
function DashboardPage() {
  ...
  const enrollmentStat = useSWR<PaginatedInterface<EnrollmentInterface>>(
    () => `/enrollments`,
    () => getEnrollments({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Enrollments',
      result: enrollmentStat?.data?.totalCount,
      isLoading: enrollmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`tutors`} passHref legacyBehavior>
                  <Button
