...
export function TutorListPage(props: TutorListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getTutors({
        relations: ['user', 'Renamedclass.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['qualification.contains', 'specialization.contains', 'availability.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TutorInterface>>(
    () => `/tutors?params=${JSON.stringify(params)}`,
    fetcher,
  );
