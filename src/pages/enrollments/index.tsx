...
import { getEnrollments, deleteEnrollmentById } from 'apiSdk/enrollments';
import { EnrollmentInterface } from 'interfaces/enrollment';

type ColumnType = ColumnDef<EnrollmentInterface, unknown>;
...
export function EnrollmentListPage(props: EnrollmentListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getEnrollments({
        relations: ['Renamedclass', 'student'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EnrollmentInterface>>(
    () => `/enrollments?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: EnrollmentInterface) => {
    if (hasAccess('enrollment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/enrollments/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.enrollment_date ? format(parseISO(record?.enrollment_date as unknown as string), 'dd-MM-yyyy') : '',
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/enrollments/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('enrollment', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/enrollments/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="enrollment" />
        </Flex>

        {hasAccess('enrollment', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/enrollments/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
