...
export function RenamedclassListPage(props: RenamedclassListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getRenamedclasses({
        relations: ['tutor', 'institute', 'enrollment.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['subject.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('tutor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'tutor',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/tutors/view/${record.tutor?.id}`}>
              {record.tutor?.qualification}
            </Link>
          ),
        }
      : null,
