...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'tutor.create': {
    roles: ['institute-owner', 'tutor'],
    key: 'new-tutor-added',
    tenantPath: ['institute', 'class', 'tutor'],
    userPath: [],
  },
  'class.create': {
    roles: ['institute-owner', 'tutor'],
    key: 'class-scheduled',
    tenantPath: ['institute', 'class'],
    userPath: [],
  },
  'enrollment.create': {
    roles: ['institute-owner', 'tutor'],
    key: 'student-enrollment',
    tenantPath: ['institute', 'class', 'enrollment'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['institute-owner'];
const customerRoles: string[] = ['student'];
const tenantRoles: string[] = ['institute-owner', 'tutor'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.institute.findFirst({
